<?php
  ...
  // +-------------------------------------------------------------------+
  // | FILE: tooltips.php                            CREATED: 02-11-2005 + 
  ...
  $zOLDAPPLE->Initialize("admin.system.tooltips", TRUE);

  // Create local classes.
  $ADMINDATA = new cSYSTEMTOOLTIPS ($zOLDAPPLE->Context);
  ...
  global $gSelectedTab;
  $gSelectedTab['admin_system_tooltips'] = 'selected';
  ...
  if ($zLOCALUSER->userAccess->a == TRUE) {
    ...
    switch ($gACTION) {
      case 'EDIT':
        $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/tooltips/edit.aobj", INCLUDE_SECURITY_NONE);
      break;
      case 'NEW':
        $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/tooltips/new.aobj", INCLUDE_SECURITY_NONE);
      break;
      case 'SELECT_ALL':
      case 'DELETE_ALL':
      case 'MOVE_UP':
      ...
      default:
        if ( ($ADMINDATA->Error == 0) OR 
          ...
          $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/tooltips/list.top.aobj", INCLUDE_SECURITY_NONE);

          // Calculate scroll values.
          $gSCROLLMAX[$zOLDAPPLE->Context] = $ADMINDATA->CountResult();

          // Adjust for a recently deleted entry.
          $zOLDAPPLE->AdjustScroll ('admin.system.tooltips', $ADMINDATA);
          ...
          for ($listcount = 0; $listcount < $gSCROLLSTEP[$zOLDAPPLE->Context]; $listcount++) {
           if ($ADMINDATA->FetchArray()) {
            ...
            $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/tooltips/list.middle.aobj", INCLUDE_SECURITY_NONE);
          ...
          $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/tooltips/list.bottom.aobj", INCLUDE_SECURITY_NONE);
  ...
  $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/frames/admin/system/tooltips.afrw", INCLUDE_SECURITY_NONE);
