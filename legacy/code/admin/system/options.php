<?php
  ...
  // +-------------------------------------------------------------------+
  // | VERSION:      0.7.9                                               |
  // | DESCRIPTION: Options administration page.                         |
  // +-------------------------------------------------------------------+
  ...
  $zOLDAPPLE->Initialize("admin.system.options", TRUE);

  // Create local classes.
  $ADMINDATA = new cSYSTEMOPTIONS ($zOLDAPPLE->Context);
  ...
  global $gSelectedTab;
  $gSelectedTab['admin_system_options'] = 'selected';
  ...
  if ($zLOCALUSER->userAccess->a == TRUE) {
    ...
    switch ($gACTION) {
      case 'EDIT':
        $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/options/edit.aobj", INCLUDE_SECURITY_NONE);
      break;
      case 'NEW':
        $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/options/new.aobj", INCLUDE_SECURITY_NONE);
      break;
      case 'SAVE':
        // Skip to the default.
      default:
        if ( ($ADMINDATA->Error == 0) OR 
          ...
          $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/options/list.top.aobj", INCLUDE_SECURITY_NONE);

          // Calculate scroll values.
          $gSCROLLMAX[$zOLDAPPLE->Context] = $ADMINDATA->CountResult();

          // Adjust for a recently deleted entry.
          $zOLDAPPLE->AdjustScroll ('admin.system.options', $ADMINDATA);
          ...
          for ($listcount = 0; $listcount < $gSCROLLSTEP[$zOLDAPPLE->Context]; $listcount++) {
           if ($ADMINDATA->FetchArray()) {
            ...
            $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/options/list.middle.aobj", INCLUDE_SECURITY_NONE);
          ...
          $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/options/list.bottom.aobj", INCLUDE_SECURITY_NONE);
  ...
  $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/frames/admin/system/options.afrw", INCLUDE_SECURITY_NONE);
