<?php
  ...
  // +-------------------------------------------------------------------+
  // | FILE: strings.php                             CREATED: 02-11-2005 + 
  ...
  $zOLDAPPLE->Initialize("admin.system.strings", TRUE);

  // Create local classes.
  $ADMINDATA = new cSYSTEMSTRINGS ($zOLDAPPLE->Context);
  ...
  global $gSelectedTab;
  $gSelectedTab['admin_system_strings'] = 'selected';
  ...
  if ($zLOCALUSER->userAccess->a == TRUE) {
    ...
    switch ($gACTION) {
      case 'EDIT':
        $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/strings/edit.aobj", INCLUDE_SECURITY_NONE);
      break;
      case 'NEW':
        $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/strings/new.aobj", INCLUDE_SECURITY_NONE);
      break;
      case 'SELECT_ALL':
      case 'DELETE_ALL':
      case 'MOVE_UP':
      ...
      default:
        if ( ($ADMINDATA->Error == 0) OR 
          ...
          $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/strings/list.top.aobj", INCLUDE_SECURITY_NONE);

          // Calculate scroll values.
          $gSCROLLMAX[$zOLDAPPLE->Context] = $ADMINDATA->CountResult();

          // Adjust for a recently deleted entry.
          $zOLDAPPLE->AdjustScroll ('admin.system.strings', $ADMINDATA);
          ...
          $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/strings/list.bottom.aobj", INCLUDE_SECURITY_NONE);
  ...
  $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/frames/admin/system/strings.afrw", INCLUDE_SECURITY_NONE);
