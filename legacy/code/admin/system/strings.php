<?php
  ...
  if ($zLOCALUSER->userAccess->a == TRUE) {
    ...
    switch ($gACTION) {
      case 'EDIT':
        $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/strings/edit.aobj", INCLUDE_SECURITY_NONE);
      break;
      case 'NEW':
        $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/strings/new.aobj", INCLUDE_SECURITY_NONE);
      break;
      case 'SELECT_ALL':
      case 'DELETE_ALL':
      case 'MOVE_UP':
      ...
      default:
        if ( ($ADMINDATA->Error == 0) OR 
          ...
          $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/strings/list.top.aobj", INCLUDE_SECURITY_NONE);

          // Calculate scroll values.
          $gSCROLLMAX[$zAPPLE->Context] = $ADMINDATA->CountResult();

          // Adjust for a recently deleted entry.
          $zAPPLE->AdjustScroll ('admin.system.strings', $ADMINDATA);
          ...
          for ($listcount = 0; $listcount < $gSCROLLSTEP[$zAPPLE->Context]; $listcount++) {
           if ($ADMINDATA->FetchArray()) {
            ...
            $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/strings/list.middle.aobj", INCLUDE_SECURITY_NONE);
          ...
          $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/strings/list.bottom.aobj", INCLUDE_SECURITY_NONE);
  ...
  $zAPPLE->IncludeFile ("$gFRAMELOCATION/frames/admin/system/strings.afrw", INCLUDE_SECURITY_NONE);
