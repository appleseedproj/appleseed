<?php
  ...
  // +-------------------------------------------------------------------+
  // | VERSION:      0.7.3                                               |
  // | DESCRIPTION: Content articles administration page.                |
  // +-------------------------------------------------------------------+
  ...
  $zOLDAPPLE->Initialize("admin.content.articles", TRUE);

  // Create local classes.
  $ADMINDATA = new cCONTENTARTICLES ($zOLDAPPLE->Context);
  ...
  global $gSelectedTab;
  $gSelectedTab['admin_content_articles'] = 'selected';
  ...
  switch ($gACTION) {
    ...
    case 'NEW':
      ...
      $gSTAMPLIST = $zHTML->SplitDate ($currently);
    break;
  ...
  if ($zLOCALUSER->userAccess->a == TRUE) {
    ...
    switch ($gACTION) {
      case 'EDIT':
        $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/content/articles/edit.aobj", INCLUDE_SECURITY_NONE);
      break;
      case 'NEW':
        $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/content/articles/new.aobj", INCLUDE_SECURITY_NONE);
      break;
      case 'SELECT_ALL':
      case 'DELETE_ALL':
      case 'MOVE_UP':
      ...
      default:
        if ( ($ADMINDATA->Error == 0) OR 
          ...
          $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/content/articles/list.top.aobj", INCLUDE_SECURITY_NONE);

          // Calculate scroll values.
          $gSCROLLMAX[$zOLDAPPLE->Context] = $ADMINDATA->CountResult();

          // Adjust for a recently deleted entry.
          $zOLDAPPLE->AdjustScroll ('admin.content.articles', $ADMINDATA);
          ...
          // Loop through the list.
          $target = "_admin/content/articles/";
          for ($listcount = 0; $listcount < $gSCROLLSTEP[$zOLDAPPLE->Context]; $listcount++) {
           if ($ADMINDATA->FetchArray()) {
            $output = $zOLDAPPLE->Format ($ADMINDATA->Full, FORMAT_VIEW);
            if ($gACTION == 'SELECT_ALL') $checked = TRUE;

            $gEXTRAPOSTDATA['ACTION'] = "EDIT"; 
            $gEXTRAPOSTDATA['tID']    = $ADMINDATA->tID;
            $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/content/articles/list.middle.aobj", INCLUDE_SECURITY_NONE);
          ...
          $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/content/articles/list.bottom.aobj", INCLUDE_SECURITY_NONE);
  ...
  $zOLDAPPLE->IncludeFile ("$gFRAMELOCATION/frames/admin/content/articles.afrw", INCLUDE_SECURITY_NONE);
