<?php
  ...
  // +-------------------------------------------------------------------+
  // | FILE: access.php                              CREATED: 02-11-2005 + 
  // | LOCATION: /code/admin/users/                 MODIFIED: 04-21-2005 +
  // +-------------------------------------------------------------------+
  ...
  $gPOSTDATA = Array ("SEARCHACCESSBY" => $gSEARCHACCESSBY,
                      "CRITERIA"        => $gCRITERIA,
  ...
  switch ($gACTION) {
    ...
    default:
      if ($gCRITERIA) {
        if ($gSEARCHACCESSBY == SQL_SKIP) {
          // Search all fields
          $ADMINDATA->SelectByAll($gCRITERIA, $gSORT, 1);
        } else {
          // Search a single field
          $ADMINDATA->Select($gSEARCHACCESSBY, $gCRITERIA, $gSORT, 1);
        } // if
  ...
  if ($zLOCALUSER->userAccess->a == TRUE) {
    ...
    switch ($gACTION) {
      case 'EDIT':
        $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/users/access/edit.aobj", INCLUDE_SECURITY_NONE);
      break;
      case 'NEW':
        $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/users/access/new.aobj", INCLUDE_SECURITY_NONE);
      break;
      case 'SAVE':
        // Skip to the default.
      default:
        if ( ($ADMINDATA->Error == 0) OR 
          ...
          $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/users/access/list.top.aobj", INCLUDE_SECURITY_NONE);

          // Calculate scroll values.
          $gSCROLLMAX[$zAPPLE->Context] = $ADMINDATA->CountResult();

          // Adjust for a recently deleted entry.
          $zAPPLE->AdjustScroll ('admin.users.access', $ADMINDATA);

          // Check if any results were found.
          if ($gSCROLLMAX[$zAPPLE->Context] == 0) {
            $zSTRINGS->Lookup ('MESSAGE.NONE', $zAPPLE->Context);
            $ADMINDATA->Message = $zSTRINGS->Output;
            $ADMINDATA->Broadcast("admin");
          } // if

          // Loop through the list.
          $target = "_admin/users/access/";
          for ($listcount = 0; $listcount < $gSCROLLSTEP[$zAPPLE->Context]; $listcount++) {
           if ($ADMINDATA->FetchArray()) {
            $PROFILEINFO->Select ("uID", $ADMINDATA->userAuth_uID, "Username DESC");
            $PROFILEINFO->FetchArray ();
            $username = $PROFILEINFO->Username;
            if ($gACTION == 'SELECT_ALL') $checked = TRUE;

            $gEXTRAPOSTDATA['ACTION'] = "EDIT"; 
            $gEXTRAPOSTDATA['tID']    = $ADMINDATA->tID;
            $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/users/access/list.middle.aobj", INCLUDE_SECURITY_NONE);
          ...
          $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/users/access/list.bottom.aobj", INCLUDE_SECURITY_NONE);
  ...
  $zAPPLE->IncludeFile ("$gFRAMELOCATION/frames/admin/users/access.afrw", INCLUDE_SECURITY_NONE);
