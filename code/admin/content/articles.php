<?php
  ...
  $zAPPLE->Initialize("admin.content.articles", TRUE);

  // Create local classes.
  $ADMINDATA = new cCONTENTARTICLES ($zAPPLE->Context);
  ...
  if ($handle = opendir('frameworks/default/templates/content/')) {
  ...
  if ($zLOCALUSER->userAccess->a == TRUE) {
    ...
    switch ($gACTION) {
      case 'EDIT':
        $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/content/articles/edit.aobj", INCLUDE_SECURITY_NONE);
      break;
      case 'NEW':
        $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/content/articles/new.aobj", INCLUDE_SECURITY_NONE);
      break;
      case 'SELECT_ALL':
      case 'DELETE_ALL':
      case 'MOVE_UP':
      ...
      default:
        if ( ($ADMINDATA->Error == 0) OR 
          ...
          $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/content/articles/list.top.aobj", INCLUDE_SECURITY_NONE);

          // Calculate scroll values.
          $gSCROLLMAX[$zAPPLE->Context] = $ADMINDATA->CountResult();

          // Adjust for a recently deleted entry.
          $zAPPLE->AdjustScroll ('admin.content.articles', $ADMINDATA);
          ...
          for ($listcount = 0; $listcount < $gSCROLLSTEP[$zAPPLE->Context]; $listcount++) {
           if ($ADMINDATA->FetchArray()) {
            $output = $zAPPLE->Format ($ADMINDATA->Full, FORMAT_VIEW);
            if ($gACTION == 'SELECT_ALL') $checked = TRUE;

            $gEXTRAPOSTDATA['ACTION'] = "EDIT"; 
            $gEXTRAPOSTDATA['tID']    = $ADMINDATA->tID;
            $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/content/articles/list.middle.aobj", INCLUDE_SECURITY_NONE);
          ...
          $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/content/articles/list.bottom.aobj", INCLUDE_SECURITY_NONE);
