<?php
  ...
  $gPOSTDATA = Array ("SEARCHTOOLTIPSBY" => $gSEARCHTOOLTIPSBY,
                      "CRITERIA"        => $gCRITERIA,
  ...
  switch ($gACTION) {
    ...
    default:
      if ($gCRITERIA) {
        if ($gSEARCHTOOLTIPSBY == SQL_SKIP) {
          // Search all fields
          $ADMINDATA->SelectByAll($gCRITERIA, $gSORT, 1);
        } else {
          // Search a single field
          $ADMINDATA->Select($gSEARCHTOOLTIPSBY, $gCRITERIA, $gSORT, 1);
        } // if
  ...
  if ($zLOCALUSER->userAccess->a == TRUE) {
    ...
    switch ($gACTION) {
      case 'EDIT':
        $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/tooltips/edit.aobj", INCLUDE_SECURITY_NONE);
      break;
      case 'NEW':
        $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/tooltips/new.aobj", INCLUDE_SECURITY_NONE);
      break;
      case 'SELECT_ALL':
      case 'DELETE_ALL':
      case 'MOVE_UP':
      ...
      default:
        if ( ($ADMINDATA->Error == 0) OR 
          ...
          $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/tooltips/list.top.aobj", INCLUDE_SECURITY_NONE);

          // Calculate scroll values.
          $gSCROLLMAX[$zAPPLE->Context] = $ADMINDATA->CountResult();

          // Adjust for a recently deleted entry.
          $zAPPLE->AdjustScroll ('admin.system.tooltips', $ADMINDATA);
          ...
          for ($listcount = 0; $listcount < $gSCROLLSTEP[$zAPPLE->Context]; $listcount++) {
           if ($ADMINDATA->FetchArray()) {
            ...
            $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/tooltips/list.middle.aobj", INCLUDE_SECURITY_NONE);
          ...
          $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/tooltips/list.bottom.aobj", INCLUDE_SECURITY_NONE);
  ...
  $zAPPLE->IncludeFile ("$gFRAMELOCATION/frames/admin/system/tooltips.afrw", INCLUDE_SECURITY_NONE);
