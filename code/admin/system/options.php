<?php
  ...
  $zAPPLE->Initialize("admin.system.options", TRUE);

  // Create local classes.
  $ADMINDATA = new cSYSTEMOPTIONS ($zAPPLE->Context);
  ...
  $gPOSTDATA = Array ("SEARCHOPTIONSBY" => $gSEARCHOPTIONSBY,
                      "CRITERIA"        => $gCRITERIA,
  ...
  switch ($gACTION) {
    ...
    default:
      if ($gCRITERIA) {
        if ($gSEARCHOPTIONSBY == SQL_SKIP) {
          // Search all fields
          $ADMINDATA->SelectByAll($gCRITERIA, $gSORT, 1);
        } else {
          // Search a single field
          $ADMINDATA->Select($gSEARCHOPTIONSBY, $gCRITERIA, $gSORT, 1);
        } // if
  ...
  if ($zLOCALUSER->userAccess->a == TRUE) {
    ...
    switch ($gACTION) {
      case 'EDIT':
        $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/options/edit.aobj", INCLUDE_SECURITY_NONE);
      break;
      case 'NEW':
        $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/options/new.aobj", INCLUDE_SECURITY_NONE);
      break;
      case 'SAVE':
        // Skip to the default.
      default:
        if ( ($ADMINDATA->Error == 0) OR 
          ...
          $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/options/list.top.aobj", INCLUDE_SECURITY_NONE);

          // Calculate scroll values.
          $gSCROLLMAX[$zAPPLE->Context] = $ADMINDATA->CountResult();

          // Adjust for a recently deleted entry.
          $zAPPLE->AdjustScroll ('admin.system.options', $ADMINDATA);
          ...
          for ($listcount = 0; $listcount < $gSCROLLSTEP[$zAPPLE->Context]; $listcount++) {
           if ($ADMINDATA->FetchArray()) {
            ...
            $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/options/list.middle.aobj", INCLUDE_SECURITY_NONE);
          ...
          $zAPPLE->IncludeFile ("$gFRAMELOCATION/objects/admin/system/options/list.bottom.aobj", INCLUDE_SECURITY_NONE);
